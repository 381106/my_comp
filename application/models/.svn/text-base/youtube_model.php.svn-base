...
class youtube_model extends CI_Model {
    ...
    function youtube_model(){
        ...
        $this->_ci->zend->load("Zend/Gdata/YouTube");
        $this->_ci->zend->load("Zend/Gdata/AuthSub");
        $this->_ci->zend->load("Zend/Gdata/App/Exception");
    }

    function _getVideoId($entry){
        return $entry->getVideoId();
    }

    function _getVideoTitle($entry){
        return htmlspecialchars($entry->getVideoTitle());
    }

    function _getVideoDescription($entry){
        return htmlspecialchars($entry->getVideoDescription());
    }

    function _getVideoCategory($entry){
        return $entry->getVideoCategory();
    }

    function _getVideoTags($entry){
        return $entry->getVideoTags();
    }

    function _getVideoRatings($entry){
        return $entry->getVideoRatingInfo();
    }

    function _getVideoDuration($entry){
        return $entry->getVideoDuration();
    }

    function _getPublished($entry){
        return $entry->getPublished()->text;
    }

    function _getVideoThumbnail($entry){
    ...
    function _getVideoEntry($videoId){
        ...
        $result['videoTitle'] = htmlspecialchars($entry->getVideoTitle());
        $result['videoUrl'] = htmlspecialchars($this->findFlashUrl($entry));
        $result['metadata'] = $this->_getVideoMetadata($entry);
        $relatedVideoFeed = $this->getRelatedVideos($entry->getVideoId());
        
        foreach ($relatedVideoFeed as $related){
            $item->thumbnail_url = htmlspecialchars($related->mediaGroup->thumbnail[0]->url);
            $item->video_id      = $related->getVideoId();
            $item->title         = process_description(htmlspecialchars($related->getVideoTitle()));
            $item->duration      = $related->getVideoDuration();
            $relatedVideoFeed_data[] = $item;
            $item = '';
        }
        $result['relatedVideoFeed'] = $relatedVideoFeed_data;

        $topRatedVideosByUser = $this->getTopRatedVideosByUser($entry->author[0]->name);
        //var_dump($topRatedVideosByUser);die;
        $topRatedVideosByUser_data = false;
        if($topRatedVideosByUser){
            foreach ($topRatedVideosByUser as $top_rated){
                $item->thumbnail_url = htmlspecialchars($top_rated->mediaGroup->thumbnail[0]->url);
                $item->video_id      = $top_rated->getVideoId();
                $item->title         = htmlspecialchars($top_rated->getVideoTitle());
                $item->duration      = $top_rated->getVideoDuration();
                $topRatedVideosByUser_data[] = $item;
                $item = '';
            }
        }
        if(!is_array($topRatedVideosByUser_data)){
            
        }
        $result['topRatedFeed'] = $topRatedVideosByUser_data;
        return $result;
    }

    function _getWatchPage($entry){
        $watchPage = htmlspecialchars($entry->getVideoWatchPageUrl());
        return $watchPage;
    }

    function _getVideoMetadata($entry){
            $result->title = htmlspecialchars($entry->getVideoTitle());
            $result->description = htmlspecialchars($entry->getVideoDescription());
            $result->authorUsername = htmlspecialchars($entry->author[0]->name);
            $result->authorUrl = 'http://www.youtube.com/profile?user=' .$result->authorUsername;
            $result->tags = htmlspecialchars(implode(', ', $entry->getVideoTags()));
            $result->duration = htmlspecialchars($entry->getVideoDuration());
            $result->watchPage = htmlspecialchars($entry->getVideoWatchPageUrl());
            $result->viewCount = htmlspecialchars($entry->getVideoViewCount());
            $result->rating = 0;
            if (isset($entry->rating->average)) {
                $result->rating = $entry->rating->average;
            }
            $numRaters = 0;
            if (isset($entry->rating->numRaters)) {
                $result->numRaters = $entry->rating->numRaters;
            }
            $result->flashUrl = htmlspecialchars($this->findFlashUrl($entry));
            return $result;
    }

    function init(){
        $this->setLogging('on');
        $this->generateUrlInformation();

        if (!isset($_POST['operation'])) {
            // if a GET variable is set then process the token upgrade
            if (isset($_GET['token'])) {
                 $this->updateAuthSubToken($_GET['token']);
            } else {
                if ( $this->loggingEnabled()) {
                     $this->logMessage('reached operations.php without $_POST or $_GET variables set', 'error');
        ...
        switch ($operation) {
            ...
            case 'create_upload_form':
                 $this->createUploadForm($_POST['videoTitle'],
            ...
            case 'edit_meta_data':
                 $this->editVideoData($_POST['newVideoTitle'],
            ...
            case 'check_upload_status':
                 $this->checkUpload($_POST['videoId']);
                break;

            case 'delete_video':
                 $this->deleteVideo($_POST['videoId']);
                break;

            case 'auth_sub_request':
                 $this->generateAuthSubRequestLink(base_include_url()."token_handler.php");
                break;
            ...
            case 'clear_session_var':
                 $this->clearSessionVar($_POST['name']);
                break;
            ...
            case 'create_playlist':
                 $this->createPlaylist($_POST['playlistTitle'], $_POST['playlistDescription']);
                break;

            case 'delete_playlist':
                 $this->deletePlaylist($_POST['playlistTitle']);
                break;

            case 'update_playlist':
                 $this->updatePlaylist($_POST['newPlaylistTitle'],
            ...
            case (strcmp(substr($operation, 0, 7), 'search_') == 0):
                ...
                $searchType = substr($operation, 7);
                 $this->searchVideos($searchType, $_POST['searchTerm'], $_POST['startIndex'],
                    $_POST['maxResults']);
                break;

            case 'show_video':
                 $this->echoVideoPlayer($_POST['videoId']);
                break;

            default:
                 $this->unsupportedOperation($_POST);
                break;
        }
    }
...
function searchVideos($searchType, $searchTerm, $startIndex, $maxResults)
{
  // create an unauthenticated service object
    ...
    if ($this->loggingEnabled()) {
        ...
        $this->logMessage($httpClient->getLastRequest(), 'request');
        $this->logMessage($httpClient->getLastResponse()->getBody(), 'response');
    }
    $res->total_results = (string)$feed->getTotalResults();
    $res->feed          = $feed;
    return $res;
    //$this->echoVideoList($feed);
}
...
{
    if(isset($this->session->userdata['loggin'])&&$this->session->userdata['loggin']=='on'){
        return true;
    }
}
...
function updateAuthSubToken($singleUseToken)
{
    ...
    header('Location: ' . base_url()."utube");
}
...
function echoVideoPlayer($videoId)
{
    ...
    $relatedVideoFeed = $this->getRelatedVideos($entry->getVideoId());
    $topRatedFeed = $this->getTopRatedVideosByUser($entry->author[0]->name);
...
END;
    ...
    echo '<br />';
    $this->echoVideoMetadata($entry);
    echo '<br /><b>Related:</b><br />';
    $this->echoThumbnails($relatedVideoFeed);
    echo '<br /><b>Top rated videos by user:</b><br />';
    $this->echoThumbnails($topRatedFeed);
}
...
function echoVideoMetadata($entry)
{
    ...
    $flashUrl = htmlspecialchars($this->findFlashUrl($entry));
    print <<<END
        <b>Title:</b> ${title}<br />
