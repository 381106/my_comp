...
class Remote_demo extends CI_Controller {
    ...
    public function html_to_pdf(){
        ...
        $this->load->view("remote_demo/html_to_pdf/index",$data);
            ...
            if($action){
                switch($action){
                    case 'render':{
                        ...
                        if(!$this->input->post('background_audio_file')){
...
$effect.":1".$wipe_effect;
                        ...
                        $command = "/usr/bin/dvd-slideshow  -mp2 -L -n ".$output_file_name." -o ".$output_dir." -f ".$command_file." 2>&1";
                        
                        if(file_exists($output_dir.$output_file_name.".vob") && filesize($output_dir.$output_file_name.".vob")){
                        ...
                        if(file_exists($output_dir.$output_file_name.".vob")){
                            chmod($output_dir, 0777);
                            $output_file_name_flv = rand(10,2000)."_slideshow.flv";
                            ...
                            $cmd = "/usr/bin/mencoder -oac copy -ovc lavc ".$output_dir.$output_file_name.".vob  -o ".$output_dir.$output_file_name_flv;
                            
                            exec($cmd.' 2>&1');
                            
                            if(file_exists($output_dir.$output_file_name_flv) && filesize($output_dir.$output_file_name_flv)){
                                chmod($output_dir.$output_file_name_flv, 0777);
                                $out = array();
                                exec('ls -la '.$output_dir.$output_file_name_flv.' 2>&1', $out);
                                $data['flash_file'] = $tmp_folder_name.$output_file_name_flv;
                                
                                $this->load->view("remote_demo/video/slideshow_step_3",$data);
                            }else{
                                echo $cmd."<br>";
                    ...
                    case 'upload_zip':{
                        ...
                        $this->load->view("remote_demo/video/slideshow_step_2",$data);
                    }
                    break;
                    ...
                    case 'flv':{
                        $data['flash_file'] = "http://194.63.157.110/dev/company/128_slideshow.flv";
                        $this->load->view("remote_demo/video/flv",$data);
                    }
                    break;
                }
                
            }else{
                $this->load->view("remote_demo/video/index",$data);
            }
